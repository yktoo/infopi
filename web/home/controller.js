(function () {
    ...
    function HomeController($interval, NsApiService, BuienRadarService, OvapiService, FxService, CryptoService, OpenHabService) {
        ...
        function updateWeather() {
            BuienRadarService.getWeather(buienRadarStationId).then(
                function (data) {
                    vm.weather = data;
                    vm.weatherErr = false;
                },
        ...
        function updateDepartureTimes() {
            NsApiService.getTrainTimes(trainDepTimesStation1).then(
                function (data) {
                    vm.trainDepartureTimes1 = data;
                    vm.trainDepartureTimes1Err = false;
                },
                function (reason) {
                    vm.trainDepartureTimes1Err = true;
                });
            NsApiService.getTrainTimes(trainDepTimesStation2).then(
                function (data) {
                    vm.trainDepartureTimes2 = data;
                    vm.trainDepartureTimes2Err = false;
                },
                function (reason) {
                    vm.trainDepartureTimes2Err = true;
                });
        }

        function updateBusData() {
            OvapiService.getBusData(ovapiBusStopCode).then(
                function (data) {
                    vm.busData = data;
                    vm.busDataErr = false;
                },
            ...
            CryptoService.getRates('BTC,ETH', 'EUR')
                .then(
                    function (data) {
                        ...
                        vm.cryptoRatesErr = false;
                    },
                    function (reason) {
                        vm.cryptoRatesErr = true;
                    })
        }
            ...
            OpenHabService(openHabServerUrl, 'gSecurity')
                .then(
                    function (data) {
                        ...
                        vm.securityDevicesErr = false;
                    },
                    function (reason) {
                        vm.securityDevicesErr = true;
                    });
        }
        ...
        function init() {
            ...
            $interval(updateNow,             1 * 1000);       // 1 sec
